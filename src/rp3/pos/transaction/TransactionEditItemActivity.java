...
public class TransactionEditItemActivity extends rp3.app.BaseActivity implements TransactionEditItemListener {
	...
	protected void onCreate(Bundle savedInstanceState) {		
		...
		if(fm.findFragmentById(R.id.content_transaction_edit_item)==null){
			
			transactionEditItemFragment = TransactionEditItemFragment.newInstance(transactionDetailId,false);
			
			getFragmentManager().beginTransaction().replace(R.id.content_transaction_edit_item,
					transactionEditItemFragment).commit();
		}
	}
	
	@Override
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(R.menu.activity_transaction_edit_item, menu);
		return true;
	}
